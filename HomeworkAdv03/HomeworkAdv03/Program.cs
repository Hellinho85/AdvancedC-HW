...
namespace HomeworkAdv03
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Module no.1: {0}", m1.Name);
            ...
            m1.SetDiscount(10); // 0%        
            Console.WriteLine($"Total Price without Discount and Currency:" +
                $" {m1.GetPrice()}.");
            Console.WriteLine($"Total Price without Discount, with Currency:" +
                $" {m1.GetPrice().PriceWithCurrency("EUR")}");
            //Console.WriteLine($"Total Price without Discount, with Currency: " +
            //    $"{m1.GetPrice().PriceWithCurrency("USD")}");
            Console.WriteLine($"Total Price with Discount and Currency:" +
                $" {m1.GetPriceWithDiscount().PriceWithCurrency("EUR")}.");
            //Console.WriteLine($"Total Price with Discount and Currency: " +
            //    $"{m1.GetPriceWithDiscount().PriceWithCurrency("USD")}.");
            Console.WriteLine("-------------------- \n");

            // Module no.2 - Peripherals;
            Module m2 = new Module("Perhipherals");
            Console.WriteLine("Module no.2: {0}", m2.Name);
            m2.AddPartToModule(p12, 2); //wifi mouse
            ...
            m2.SetDiscount(5); // 5%
            Console.WriteLine($"Total Price without Discount and without Currency:" +
                $" {m2.GetPrice()}.");
            Console.WriteLine($"Total Price without Discount, with Currency:" +
                $" {m2.GetPrice().PriceWithCurrency("EUR")}.");
            //Console.WriteLine($"Total Price without Discount, with Currency: " +
            //    $"{m2.GetPrice().PriceWithCurrency("USD")}.");
            Console.WriteLine($"Total Price with Discount and Currency:" +
                $" {m2.GetPriceWithDiscount().PriceWithCurrency("EUR")}.");
            //Console.WriteLine($"Total Price with Discount and Currency:" +
            //    $" {m2.GetPriceWithDiscount().PriceWithCurrency("USD")}.");
            Console.WriteLine("-------------------- \n");

            // PC Configuration;

            PCConfiguration pc1 = new PCConfiguration(Enums.Colors.Blue); //pc configuration
            pc1.Name = "PC Configuration no.1";
            ...
            Console.WriteLine($"Total Price without Discount and without Currency:" +
                $" {pc1.GetPrice()}.");
            Console.WriteLine($"Total Price without Discount, with Currency:" +
                $" {pc1.GetPrice().PriceWithCurrency("EUR")}.");
            Console.WriteLine($"Total Price with Discount and with Currency:" +
                $" {pc1.GetPriceWithDiscount().PriceWithCurrency("EUR")}.");
            //Console.WriteLine($" Total Price with Discount and with Currency:" +
            //    $" {pc1.GetPriceWithDiscount().PriceWithCurrency("USD")}.");
            Console.WriteLine("That is one monster PC you just built!!! :P");
            Console.WriteLine("Have a pleasant day and please come again!");
            Console.WriteLine("--------------------");
